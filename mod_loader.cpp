...
void Mod::load_item_info(const string &filename)
{ // loads object info from file
    ...
    if (fil)
    {
        ...
        { // object name
            ...
            while (strcmp(stripped_fgets(rivi, sizeof(rivi), fil), "end_effects_block") != 0)
            {
                ...
                if (temp_effect.effect.sound_name != "none")
                    temp_effect.effect.sound = resources->load_sample(rivi, 2, mod_name); // hit sound
                ...
                temp_effect.key_description = stripped_fgets(rivi, sizeof(rivi), fil);
...
void Mod::load_object_info(const string &filename)
{ // loads object info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::load_creature_info(const string &filename)
{ // loads object info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::load_weapon_info(const string &filename)
{ // loads object info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::load_climate_info(const string &filename)
{ // loads climate info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
    ...
    if (fil)
    {
        ...
        while (strcmp(stripped_fgets(rivi, sizeof(rivi), fil), "end_of_file") != 0)
        {
            ...
            stripped_fgets(rivi, sizeof(rivi), fil);
            while (strcmp(stripped_fgets(rivi, sizeof(rivi), fil), "end_of_terrains") != 0)
            { // terrains
            ...
            stripped_fgets(rivi, sizeof(rivi), fil);
            while (strcmp(stripped_fgets(rivi, sizeof(rivi), fil), "end_of_props") != 0)
            { // props
...
void Mod::load_area_info(const string &filename)
{ // loads area info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::load_light_info(const string &filename)
{ // loads light info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::load_plot_object_info(const string &filename)
{ // loads object info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
    ...
    if (fil)
    {
        ...
        { // object name
            ...
            if (temp_object.sound_name != "none")
                temp_object.sound = resources->load_sample(rivi, 8, mod_name);
            ...
            while (strcmp(stripped_fgets(rivi, sizeof(rivi), fil), "end_effects_block") != 0)
            {
                ...
                if (temp_effect.sound_name != "none")
                    temp_effect.sound = resources->load_sample(rivi, 2, mod_name); // hit sound
...
void Mod::load_animation_info(const string &filename)
{ // loads object info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::load_race_info(const string &filename)
{ // loads race info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
    ...
    if (fil)
    {
        ...
        { // name
            ...
            if (temp_race.maximum_carry_weight <= 0)
                temp_race.maximum_carry_weight = 0.001f;
...
void Mod::load_polygons(const string &filename)
{ // loads polygon info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::grow_polygon(polygon_base *temp_polygon)
{
    ...
    for (a = 0; a < temp_polygon->points.size() - 1; a++)
    {
        ...
        int previous = a - 1;
        if (previous < 0)
            previous = temp_polygon->points.size() - 2;
...
void Mod::load_AI_side(const string &filename)
{ // loads AI info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::load_bars(const string &filename)
{ // loads bar info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::load_AI_info(const string &filename)
{ // loads AI info from file
    debug->debug_output("Load file " + filename, Action::START, Logfile::STARTUP);
...
void Mod::save_terrain_maps(string filename)
{
    ...
    if (filename == "")
        filename = "data/" + mod_name + "/terrain_maps.dat";
...
void Mod::load_sector_presets(const std::string &filename)
{
    ...
    while (true)
    {
        ...
        if (!name_line || strstr(name_line, "end_of_file"))
            break;
        ...
        if (tag_start && strstr(tag_start, "begin_tags"))
        {
            while (true)
            {
                ...
                if (!tag_line || strstr(tag_line, "end_tags"))
                    break;
...
void Mod::load_loot_tables(const std::string &filename)
{
    ...
    char line[512];
    LootTable *currentTable = nullptr;
    int lastID = -1;
    bool inItems = false;
    ...
    while (fgets(line, 512, f))
    {
        ...
        if (s.empty() || s == "end_of_file")
            continue;

        if (s == "begin_table")
        {
            // FIX: Accessing the member vector 'loot_tables' defined in mod_loader.h
            loot_tables.push_back(LootTable());
            currentTable = &loot_tables.back();
            continue;
        }
        ...
        if (currentTable)
        {
            if (inItems)
            {
                ...
                {
                    // Accessing currentTable which is a pointer to a LootTable struct
                    currentTable->entries.push_back({lastID, value});
...
void Mod::load_mod(const string &mod, debugger *debugger, resource_handler *resources)
{
    ...
    load_loot_tables("data/" + mod_name + "/loot_tables.dat");
