...
const std::string game_version = GAME_VERSION; //"1.3.4.1";
...
bool game_engine::Frame(void)
{
    ...
    if (override_mousex < 0)
        override_mousex = 0;
    if (override_mousey < 0)
        override_mousey = 0;
    if (override_mousex > screen_width)
        override_mousex = screen_width;
    if (override_mousey > screen_height)
        override_mousey = screen_height;
...
bool game_engine::cfg_load(void)
{
    ...
    screen_width = atoi(temprivi);
    fgets(temprivi, sizeof(temprivi), fil);
    screen_height = atoi(temprivi);
    fgets(temprivi, sizeof(temprivi), fil);
    bit_depth = atoi(temprivi);
    fgets(temprivi, sizeof(temprivi), fil);
    windowed = atoi(temprivi);
    fgets(temprivi, sizeof(temprivi), fil);
    play_sound = strtobool(temprivi);
    fgets(temprivi, sizeof(temprivi), fil);
...
void game_engine::initialize_game(void)
...
void game_engine::uninitialize_game(void)
{ // uninitialize game
...
void game_engine::render_map(void)
{ // renders game map
    ...
    if (debugging)
    {
        ...
        if (sid != last_logged_sid || subsid != last_logged_subsid)
        {
            ...
            if (subsid > 0)
            {
                ...
                for (auto &inst : map_main->sector_instances)
                { // Or subsector_instances if separate
                    if (inst.id == subsid)
                    {
    ...
    paused = false;
    if (ask_quit)
        paused = true; // game paused by p
    if (ask_continue_game)
        paused = true; // paused after loading
    if (pop_up_mode == 0)
        paused = true; // text showing
    if (pop_up_mode == 3)
        paused = true; // item mode
    ...
    if ((screen_height == 480) || (screen_height == 600))
    {
        loppu_x += 1;
        loppu_y += 2;
    }
    if ((screen_height == 1200) || (screen_height == 1600))
    {
        loppu_x += 1;
        loppu_y += 1;
    }
    ...
    if (screen_start_y < 0)
        screen_start_y = 0;
    if (screen_end_x > map_main->sizex - 1)
        screen_end_x = map_main->sizex - 1;
    if (screen_end_y > map_main->sizey - 1)
        screen_end_y = map_main->sizey - 1;
    ...
    if (!paused)
    {
        if (!slider_active)
        {
            ...
            if (day_timer > mod.general_races[player_race].day_speed)
            {
                ...
                {
                    day_number++;
                    if (record_message(0, day_number))
                    { // record the journal showing
                        if (show_journals)
...
void game_engine::draw_map_grid(void)
{ // renders map grid
    int k;
    int texture;
...
void game_engine::draw_map_objects(int layer)
{ // renders map grid
    // return;
    int i, j, k;
    grim->Quads_SetSubset(0, 0, 1, 1);
    ...
    if (alku_x < 0)
        alku_x = 0;
    if (loppu_x > map_main->sizex - 1)
        loppu_x = map_main->sizex;
    if (alku_y < 0)
        alku_y = 0;
    if (loppu_y > map_main->sizey - 1)
        loppu_y = map_main->sizey;
...
bool game_engine::draw_object(map_object *object, int layer, float object_x, float object_y)
{
    ...
    if (mod.general_objects[object->type].layer != layer)
        return false;
    // dead
    if (object->dead)
        return false;
    ...
    if (-camera_x + x < -size * 1.415f)
    {
        return false;
    }
    if (-camera_x + x > screen_width + size * 0.415f)
    {
        return false;
    }
    if (-camera_y + y < -size * 1.415f)
    {
        return false;
    }
    if (-camera_y + y > screen_height + size * 0.415f)
    {
        return false;
    }
...
bool game_engine::draw_item(map_object *object, int layer, float object_x, float object_y)
{
    ...
    // check if it's visible
    if (-camera_x + object_x < -size * 1.415f)
    {
        return false;
    }
    if (-camera_x + object_x > screen_width + size * 0.415f)
    {
        return false;
    }
    if (-camera_y + object_y < -size * 1.415f)
    {
        return false;
    }
    if (-camera_y + object_y > screen_height + size * 0.415f)
    {
        return false;
    }
...
void game_engine::draw_map_creatures(int layer)
{ // draws the creatures
    unsigned int i, j, k;
    int creature;
    float x0, y0, x1, y1;
    ...
    if (alku_x < 0)
        alku_x = 0;
    if (loppu_x > map_main->sizex - 1)
        loppu_x = map_main->sizex - 1;
    if (alku_y < 0)
        alku_y = 0;
    if (loppu_y > map_main->sizey - 1)
        loppu_y = map_main->sizey - 1;
    ...
    for (i = alku_x; i < loppu_x; i++)
    {
        for (j = alku_y; j < loppu_y; j++)
        {
            for (k = 0; k < map_main->at(i, j).total_creatures; k++)
            {
                ...
                if (map_main->creature[creature].alpha == 0)
                    continue;
                ...
                if (map_main->creature[creature].alpha > 0.5f)
                    if (mod.general_creatures[map_main->creature[creature].type].bars_visible)
                        if (map_main->creature[creature].show_energy)
                        {
                            ...
                            {
                                if (!map_main->creature[creature].bars[meter].active)
                                    continue;
                                if (mod.general_bars[meter].visible_on_enemies == 0)
                                    continue;
...
vector<int> game_engine::AI_list_thinkers(void)
{
    ...
    for (i = 0; i < map_main->creature.size(); i++)
    {
        ...
        // visible, add to list
        if ((map_main->creature[creature].x > camera_x - grid_size) && (map_main->creature[creature].x < camera_x + screen_width + grid_size) && (map_main->creature[creature].y > camera_y - grid_size) && (map_main->creature[creature].y < camera_y + screen_height + grid_size))
        {
...
void game_engine::AI_find_target(int index, int *enemy_index, float *enemy_distance, int *friend_index, float *friend_distance)
{
    ...
    *friend_distance = 1000000000;
    ...
    for (i = AI_thinkers.size() - 1; i >= 0; i--)
    {
        ...
        if (map_main->creature[creature_2].dead)
            continue;
        if (map_main->creature[creature_2].killed)
            continue;
        ...
        // enemy
        else if (!mod.AI_sides[map_main->creature[index].side].friend_with_side[map_main->creature[creature_2].side])
        {
            // not a threat, don't attack
            if ((mod.general_creatures[map_main->creature[creature_2].type].weapon == 0) && (creature_2 != player_controlled_creature))
                continue;
    ...
    case 1:
        if (elapsed * game_speed > 1)
            if (randInt(0, (3000) / (int)(elapsed * game_speed)) == 0)
            {
                ...
                if (map_main->creature[creature].move_to_x < 0)
                    map_main->creature[creature].move_to_x = map_main->sizex * general_object_size * 0.5f;
                if (map_main->creature[creature].move_to_y < 0)
                    map_main->creature[creature].move_to_y = map_main->sizey * general_object_size * 0.5f;
                if (map_main->creature[creature].move_to_x > map_main->sizex * general_object_size)
                    map_main->creature[creature].move_to_x = map_main->sizex * general_object_size * 0.5f;
                if (map_main->creature[creature].move_to_y > map_main->sizey * general_object_size)
                    map_main->creature[creature].move_to_y = map_main->sizey * general_object_size * 0.5f;
            }
        break;
...
bool game_engine::AI_eat_behavior(int creature)
{
    ...
    if (mod.general_creatures[map_main->creature[creature].type].eat_item >= 0)
    {
        ...
        if (start_x < 0)
            start_x = 0;
        if (end_x > map_main->sizex - 1)
            end_x = map_main->sizex - 1;
        if (start_y < 0)
            start_y = 0;
        if (end_y > map_main->sizey - 1)
            end_y = map_main->sizey - 1;
        ...
        for (a = start_x; a < end_x; a++)
        {
            for (b = start_y; b < end_y; b++)
            {
                for (auto &item : map_main->at(a, b).items)
                {
                    ...
                    if (this_creature.dead)
                        continue;
                    if (this_creature.base_type != 1)
                        continue;
                    if (this_creature.type == mod.general_creatures[map_main->creature[creature].type].eat_item)
                    {

                        float distance = sqr(creature_x - (this_creature.x + this_creature.size * general_object_size * 0.5f)) + sqr(creature_y - (this_creature.y + this_creature.size * general_object_size * 0.5f));
        ...
        if ((nearest_item >= 0) && (nearest_distance < sqr(512)))
        {
            ...
            if (nearest_distance < 100)
            {
                ...
                if (map_main->items[nearest_item].amount == 0)
                    map_main->items[nearest_item].dead = true;
        ...
        if ((creature_x != map_main->creature[creature].move_to_x) || (creature_y != map_main->creature[creature].move_to_y))
        {
            ...
            else
            {
                ...
                if (distance_to_waypoint > 100)
                {
                    ...
                    if (map_main->creature[creature].rotation_legs - dq > pi)
                        temp1 = temp1 - pi * 2;
                    if (dq - map_main->creature[creature].rotation_legs > pi)
                        dq = dq - pi * 2;
    ...
    else
    {
        if ((creature_x != map_main->creature[creature].move_to_x) || (creature_y != map_main->creature[creature].move_to_y))
        {
            ...
            if (distance_to_waypoint > 100)
            {
                ...
                if (map_main->creature[creature].rotation - dq > pi)
                    temp1 = temp1 - pi * 2;
                if (dq - map_main->creature[creature].rotation > pi)
                    dq = dq - pi * 2;
                float angle_to_waypoint = (float)fabs(dq - temp1);
...
float game_engine::AI_pathfinding(int creature)
{
    ...
    for (a = 0; a < lines; a++)
    {
        ...
        if (!mod.terrain_types[map_main->at(static_cast<int>(creature_x / general_object_size), static_cast<int>(creature_y / general_object_size)).terrain_type].AI_avoid)
        {
            ...
            for (b = 0; b < collisions2.size(); b++)
                collisions.push_back(collisions2[b]);
        }
...
void game_engine::creature_actions_loop(void)
{
    ...
    for (i = 0; i < AI_thinkers.size(); i++)
    {
        ...
        visible = ((map_main->creature[creature].x > camera_x - grid_size) && (map_main->creature[creature].x < camera_x + screen_width + grid_size) && (map_main->creature[creature].y > camera_y - grid_size) && (map_main->creature[creature].y < camera_y + screen_height + grid_size));
...
bool game_engine::creature_actions(const int creature, const bool visible)
{ // calculates the creature actions and animation
...
*/
    ...
    else
    {
        ...
        if (thiscreature.alpha < 0)
            thiscreature.alpha = 0;
    }
    ...
    if (creature != 0) // only for non-player creatures
        if (!thiscreature.killed)
        {
            ...
            for (int meter = 0; meter < maximum_bars; meter++)
            {
                if (!thiscreature.bars[meter].active)
                    continue;
            ...
            if (mod.general_creatures[thiscreature.type].die_after_seconds > 0)
            {
                ...
                if (thiscreature.life_time > mod.general_creatures[thiscreature.type].die_after_seconds * 1000)
                {
                    ...
                    for (c = 0; c < mod.general_creatures[thiscreature.type].death_block.size(); c++)
                    {
                        ...
                        if (effect.death_type != 1)
                            continue; // only on timed death
            ...
            if (die_now)
            {
                ...
                for (c = 0; c < mod.general_creatures[thiscreature.type].death_block.size(); c++)
                {
                    ...
                    if ((effect.death_type == 0) || ((effect.death_type == 2) && (thiscreature.last_bullet_hit_from_creature_number == 0)))
                    { // only on any death
                ...
                for (a = 0; a < AI_thinkers.size(); a++)
                {
                    ...
                    if (map_main->creature[creature2].side == thiscreature.side)
                        continue;
                    if (sqr(map_main->creature[creature2].x - thiscreature.x) + sqr(map_main->creature[creature2].y - thiscreature.y) > sqr(500))
                        continue;
    ...
    if (!thiscreature.killed)
        for (c = 0; c < mod.general_creatures[thiscreature.type].timed_block.size(); c++)
        {
            ...
            if ((time_from_beginning - thiscreature.script_timer[c]) * 1000 < effect.interval)
                continue;
            ...
            if (thiscreature.fire_timer < 0)
            {
                ...
                if (!bullet_shot.dead)
                {
                    ...
                    for (a = 0; a < AI_thinkers.size(); a++)
                    {
                        ...
                        if (sqr(map_main->creature[creature2].x - thiscreature.x) + sqr(map_main->creature[creature2].y - thiscreature.y) > sqr(hear_distance))
                            continue;
    ...
    if (!thiscreature.killed)
    {
        ...
        if (thiscreature.x < left_map_limit)
            thiscreature.x = left_map_limit;
        if (thiscreature.y < top_map_limit)
            thiscreature.y = top_map_limit;
        if (thiscreature.x > right_map_limit)
            thiscreature.x = right_map_limit;
        if (thiscreature.y > bottom_map_limit)
            thiscreature.y = bottom_map_limit;
        assert(!isnan(thiscreature.x));
        assert(!isnan(thiscreature.y));
        ...
        for (a = 0; a < mod.terrain_types[terrain_type].effects.size(); a++)
        {
            ...
            if (time_from_beginning - terrain_timers[terrain_type].subtype[a] < mod.terrain_types[terrain_type].effects[a].interval * 0.001f)
                continue;
        ...
        if (thiscreature.up > 0)
        {
            ...
            if (thiscreature.backward_forward_speed > 1)
                thiscreature.backward_forward_speed = 1;
        }
        // down
        ...
        if ((thiscreature.up == 0) && (thiscreature.down == 0))
        {
            ...
            {
                thiscreature.backward_forward_speed -= movement_change_speed * elapsed * game_speed * fabs(thiscreature.backward_forward_speed) / thiscreature.backward_forward_speed;
            }
            // thiscreature.backward_forward_speed=0;
        }

        // keep rotation in 0>x<2pi
        if (thiscreature.rotation_legs < 0)
            thiscreature.rotation_legs += pi * 2;
        if (thiscreature.rotation_legs > 2 * pi)
            thiscreature.rotation_legs -= pi * 2;
        if (thiscreature.rotation < 0)
            thiscreature.rotation += pi * 2;
        if (thiscreature.rotation > 2 * pi)
            thiscreature.rotation -= pi * 2;
        if (thiscreature.rotation_head < 0)
            thiscreature.rotation_head += pi * 2;
        if (thiscreature.rotation_head > 2 * pi)
            thiscreature.rotation_head -= pi * 2;
            ...
            for (a = start_x; a < end_x; a++)
            {
                for (b = start_y; b < end_y; b++)
                {
                    if (map_main->at(a, b).total_creatures > 0)
                        ...
                        {
                            if (map_main->at(a, b).creatures[c] == creature)
                                continue;
                            if (map_main->creature[map_main->at(a, b).creatures[c]].dead)
                                continue;
                            if (map_main->creature[map_main->at(a, b).creatures[c]].killed)
                                continue; // killed
                            if ((map_main->creature[map_main->at(a, b).creatures[c]].side == map_main->creature[0].side) && (creature == 0))
                                continue; // friends don't push player
        ...
        if (!thiscreature.killed)
        {
            ...
            if (!thiscreature.killed)
                if ((thiscreature.x2 != thiscreature.x) || (thiscreature.y2 != thiscreature.y) || (creature == 0))
                {
                    ...
                    for (a = alku_x; a < loppu_x; a++)
                    {
                        for (b = alku_y; b < loppu_y; b++)
                        {
                            ...
                            if (!prop_prevented)
                                ...
                                {
                                    if (map_main->object[map_main->at(a, b).objects[c]].dead)
                                        continue;
                            ...
                            if (!plot_object_prevented)
                                ...
                                {
                                    if (map_main->items[map_main->at(a, b).items[c]].dead)
                                        continue;
                                    if (map_main->items[map_main->at(a, b).items[c]].base_type != 0)
                                        continue;
        ...
        if (!thiscreature.killed)
            if (thiscreature.animation[1] < 10)
            {
                ...
                if (thiscreature.animation_timer[1] < 0)
                    thiscreature.animation_timer[1] = 0;
...
void game_engine::player_controls(int control_type)
{
    ...
    if (control_type == 2)
    {
        ...
        if (grim->Key_Down(KEY_DOWN) || grim->Key_Down(KEY_S))
            map_main->creature[player_controlled_creature].down = true;
        if (grim->Key_Down(KEY_LEFT) || grim->Key_Down(KEY_A))
        {
    ...
    if (target != -1)
    {
        ...
        if (map_main->creature[player_controlled_creature].rotation_legs - dq > pi)
            temp1 = temp1 - pi * 2;
        if (dq - map_main->creature[player_controlled_creature].rotation_legs > pi)
            dq = dq - pi * 2;
        int negaatio = 1;
        if (dq < temp1)
            negaatio = -1;
    ...
    if (map_main->creature[player_controlled_creature].rotation < 0)
        map_main->creature[player_controlled_creature].rotation += pi * 2;
    if (map_main->creature[player_controlled_creature].rotation > 2 * pi)
        map_main->creature[player_controlled_creature].rotation -= pi * 2;
    ...
    if ((mouse_wheel > 0) || (key_e && !key_e2))
    {
        ...
        while (true)
        {
            passes++;
            a++;
            if (a >= inventory[active_inventory].player_items.size())
                a = 0;
            if (passes > inventory[active_inventory].player_items.size())
                break;
            Mod::effect effect;
    ...
    if (mouse_wheel < 0)
    {
        ...
        while (true)
        {
            ...
            a--;
            if (a < 0)
                a = inventory[active_inventory].player_items.size() - 1;
            if (passes > inventory[active_inventory].player_items.size())
                break;
...
void game_engine::load_setup(const char *filename)
{ // loads setup from file
...
void game_engine::save_setup(const char *filename)
{ // saves setup to file
            ...
            // creature collision detection
            {
                ...
                for (a = 0; a < collisions.size(); a++)
                {
                    ...
                    if (collisions[a].type == 0)
                    {
                        ...
                        if (map_main->creature[collisions[a].subtype].side == (*it).side)
                            continue;

                        // rule out the shooter
                        if (collisions[a].subtype == (*it).from_creature)
                            continue;
                            ...
                            if (temp_effect.effect_number == 4)
                            {
                                ...
                                if (temp_effect.parameter1 < 0)
                                    if (collisions[a].subtype == 0)
                                    {
                                        ...
                                        if (temp_effect.parameter1 > -1)
                                            temp_effect.parameter1 = -1;
                                    }
...
void game_engine::calculate_lights(void)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if (daylight[i] < mod.general_climates[climate].light_min_r)
            daylight[i] = mod.general_climates[climate].light_min_r;
        if (daylight[i] > mod.general_climates[climate].light_max_r)
            daylight[i] = mod.general_climates[climate].light_max_r;
    }
    ...
    for (int i = 0; i < (int)map_main->lights.size(); i++)
    {
        ...
        {
            // --- MODE A: CHAOTIC FLICKER (Fires / Broken Lights) ---
            if (L_temp.pulsating >= 2.0f)
            {
                // Control how often it jumps based on the pulse speed.
                // pulsating = 2.0 jumps roughly twice a second. pulsating = 10.0 jumps constantly.
                int flicker_chance = (int)(20.0f / L_temp.pulsating);
                if (flicker_chance < 1)
                    flicker_chance = 1;

                if (randInt(0, flicker_chance) == 0)
                {
                    l.pulse = randDouble(0.75f, 1.05f); // Slightly softer size jumps
                }
            }
            // --- MODE B: SMOOTH PULSE (Beacons / Monitors) ---
            else
            {
                // The * 0.002f converts milliseconds into a nice, smooth speed scale.
                // At pulsating = 1.0, the light will smoothly breathe over ~3 seconds.
                l.pulse = 1.0f + sincos.table_sin(internal_timer * 0.002f * L_temp.pulsating + l.pulse_phase) * 0.15f;
            }
        }
        else
        {
        ...
        // Safety clamp to prevent negative or massive light sizes
        if (l.pulse < 0.1f)
            l.pulse = 0.1f;
        if (l.pulse > 2.0f)
            l.pulse = 2.0f;
...
void game_engine::make_particle(int type, int layer, float time, float x, float y, float vx, float vy, float angle) // particle animation starter
{
    ...
    if (x - camera_x < 0)
        return;
    if (x - camera_x > screen_width)
        return;
    if (y - camera_y < 0)
        return;
    if (y - camera_y > screen_height)
        return;
...
void game_engine::draw_particles(int layer)
{ // draws particles
    ...
    for (a = 0; a < particles.size(); a++)
    {
        ...
        for (it = particles[a].particles_list[layer].begin(); it != particles[a].particles_list[layer].end();)
        {
            ...
            if ((*it).x - camera_x < -500)
            {
                it = particles[a].particles_list[layer].erase(it);
                continue;
            }
            if ((*it).x - camera_x > screen_width + 500)
            {
                it = particles[a].particles_list[layer].erase(it);
                continue;
            }
            if ((*it).y - camera_y < -500)
            {
                it = particles[a].particles_list[layer].erase(it);
                continue;
            }
            if ((*it).y - camera_y > screen_height + 500)
            {
                it = particles[a].particles_list[layer].erase(it);
                continue;
            }
            ...
            if (time_changes_size)
            {
                ...
                if (size > particle_type_size)
                    size = particle_type_size;
            }
            if (size < 1)
            {
                it++;
...
void game_engine::draw_mouse(int cursor, float hot_spot_x, float hot_spot_y, float r, float g, float b)
{ // draws mouse
...
void game_engine::create_plot_object(map *map_to_edit, int general_item_class, float x, float y, float angle, float size)
{ // throws in the items to the map
    ...
    */
    ...
    if ((x < 0) || (y < 0))
    {
        ...
        if (mod.general_plot_objects[a].location_type == 2)
        {
            ...
            {
                if (map_to_edit->items[d].dead)
                    continue;
                if (map_to_edit->items[d].base_type != 0)
                    continue;
                if (map_to_edit->items[d].item_type != mod.general_plot_objects[a].location_parameter0)
                    continue;
...
void game_engine::create_item(map *map_to_edit, int item_type, int amount, float x, float y, float rotation)
{ // throws in the items to the map
...
void game_engine::draw_pop_up(void)
{
    ...
    if ((mousex >= pop_up_x + 192 * x_multiplier) && (mousex < pop_up_x + 218 * x_multiplier))
    {
        if ((mousey >= pop_up_y + 224 * y_multiplier) && (mousey < pop_up_y + 249 * y_multiplier))
        {
            additional_info = true;
            if (pop_up_mode == 1)
                add_info = "Right click to review journal";
            // click
            if (!mouse_right && mouse_right2)
            {
                playsound(UI_game_click[0], 1, 0, 0, 0, 0);
    ...
    if (show_radar)
    {
        ...
        if (pop_up_mode == 1)
        {
            ...
            if (proposed_map_widthx < map_widthx)
                map_widthx = proposed_map_widthx;
            if (proposed_map_widthy < map_widthy)
                map_widthy = proposed_map_widthy;
                    ...
                    for (j = alku_y; j < loppu_y; j++)
                    {
                        if (map_main->at(i, j).total_creatures > 0)
                            for (k = 0; k < map_main->at(i, j).total_creatures; k++)
                            {
                                ...
                                if (mod.general_creatures[map_main->creature[creature].type].particle_on_radar < 0)
                                    continue;
            ...
            // scanner
            {
                ...
                for (int i = 0; i < map_main->items.size(); i++)
                {
                    ...
                    {
                        if (map_main->items[i].event_used)
                            continue;
                        if (map_main->items[i].type == 0)
                            continue; // null object visual
                    ...
                    if (show_on_type != 0)
                    {
                        ...
                        if ((scanner_active == 0) && (show_on_type == 1))
                            continue;
...
void game_engine::calculate_items(void)
{
    ...
    for (i = alku_x; i < loppu_x; i++)
    {
        for (j = alku_y; j < loppu_y; j++)
        {
            for (k = 0; k < map_main->at(i, j).items.size(); k++)
            {
                if (map_main->items[map_main->at(i, j).items[k]].dead)
                    continue;
            ...
            for (j = alku_y; j < loppu_y; j++)
            {
                for (k = 0; k < map_main->at(i, j).items.size(); k++)
                {
                    ...
                    if (map_main->items[item].base_type == 1)
                    {
                        ...
                        if (object_x < grid_size)
                        {
                            map_main->items[item].x = grid_size - size * 0.5f;
                        }
                        if (object_y < grid_size)
                        {
                            map_main->items[item].y = grid_size - size * 0.5f;
                        }
                        if (object_x > (map_main->sizex - 1) * grid_size - grid_size)
                        {
                            map_main->items[item].x = (map_main->sizex - 1) * grid_size - grid_size - size * 0.5f;
                        }
                        if (object_y > (map_main->sizey - 1) * grid_size - grid_size)
                        {
                            map_main->items[item].y = (map_main->sizey - 1) * grid_size - grid_size - size * 0.5f;
                        }
...
void game_engine::delete_plot_object(map *edit_map, int number, int i, int j, int k)
{
    ...
    if (k < 0)
        return;
    if (k >= edit_map->at(i, j).items.size())
        return;
...
int game_engine::delete_item(int number)
{
    ...
    if (number < 0)
        return 0;
    if (number >= inventory[active_inventory].player_items.size())
        return number;
...
bool game_engine::use_item(int general_item_number, int *item_number_in_list, Mod::effect_base effect, bool unuse, bool output, bool just_asking, bool check_slots)
{
    ...
    for (b = 0; b < mod.general_races[player_race].disabled_item_classes.size(); b++)
    {
        ...
        {
            if (output)
                text_manager.message(5000, 2000, mod.general_races[player_race].disabled_item_classes_text[b]);
            return false;
        }
    }
    ...
    if (mod.general_items[general_item_number].wield_slots.size() > 0)
    {
        ...
        if (!unuse)
        {
            ...
            for (b = 0; b < mod.general_items[general_item_number].wield_slots.size(); b++)
            {
                ...
                if (mod.general_items[general_item_number].wield_slots[b] >= mod.general_races[player_race].slots.size())
                    return false;
                ...
                {
                    if (output)
                        text_manager.message(3000, 1000, "You cannot use this.");
                    return false;
                }
                ...
                if (!just_asking || check_slots) // if we're just asking if it can be used, we're not interested if the slot is free or not
                    ...
                    {
                        if (output)
                            text_manager.message(3000, 1000, "Slot not available.");
                        return false;
                    }
            }
        }
...
bool game_engine::run_effect(Mod::effect effect, creature_base *creature, int creature_number, float x, float y, float angle, bool undo)
{
                ...
                if (effect.parameter4 > 0)
                {
                    ...
                    for (int a = alku_x; a < loppu_x; a++)
                    {
                        for (int b = alku_y; b < loppu_y; b++)
                        {
                            ...
                            {
                                if (map_main->items[map_main->at(a, b).items[c]].base_type != 1)
                                    continue;
                                if (map_main->items[map_main->at(a, b).items[c]].dead)
                                    continue;
                                float size = map_main->items[map_main->at(a, b).items[c]].size * general_object_size;
                                if (map_main->items[map_main->at(a, b).items[c]].x + size * 0.5f < x - effect.parameter3)
                                    continue;
                                if (map_main->items[map_main->at(a, b).items[c]].x + size * 0.5f > x + effect.parameter3)
                                    continue;
                                if (map_main->items[map_main->at(a, b).items[c]].y + size * 0.5f < y - effect.parameter3)
                                    continue;
                                if (map_main->items[map_main->at(a, b).items[c]].y + size * 0.5f > y + effect.parameter3)
                                    continue;
    ...
    case 21:
        ...
        if (!undo)
        {
            ...
            else
            {
                ...
                {
                    if (map_storage[effect.parameter1]->items[a].dead)
                        continue;
        ...
        if (!undo)
        {
            ...
            if (effect.parameter3 == 1)
            {
                ...
                else
                {
                    ...
                    {
                        if (a == creature_number)
                            continue; // self
                        if (map_main->creature[a].dead)
                            continue;
                        if (map_main->creature[a].killed)
                            continue;
                        if (mod.AI_sides[map_main->creature[creature_number].side].friend_with_side[map_main->creature[a].side])
                            continue;
            ...
            if (effect.parameter3 == 5)
            {
                // Directly use player (creature 0) rotation
                fire_angle = map_main->creature[0].rotation_head;
            }
                    ...
                    for (int c = 0; c < map_main->at(a, b).items.size(); c++)
                    {
                        ...
                        if (map_main->items[map_main->at(a, b).items[c]].item_type == effect.parameter1)
                        {
                            ...
                            for (int d = 0; d < amount_was; d++)
                            {
                                ...
                                if (effect.parameter3 > 0)
                                    if (deleted_amount >= effect.parameter3)
                                        break;
                            }
        ...
        if (!undo && (creature->carried_creature == -1))
        {
            ...
            for (int a = 0; a < map_main->creature.size(); a++)
            {
                ...
                if ((int)effect.parameter1 == 0)
                    if (map_main->creature[a].type != (int)effect.parameter2)
                        continue;
                if ((int)effect.parameter1 == 1)
                    if (mod.general_creatures[map_main->creature[a].type].creature_class != (int)effect.parameter2)
                        continue;
                if ((int)effect.parameter4 == -1)
                    if (map_main->creature[a].side != creature->side)
                        continue;
                if ((int)effect.parameter4 >= 0)
                    if (map_main->creature[a].side != (int)effect.parameter4)
                        continue;
                ...
                if (distance > sqr(effect.parameter3))
                    continue;
    ...
    case 70:
    {
        ...
        { // Sector First (fallback to sub)
            target_inst_id = (tile.sector_id > 0) ? tile.sector_id : tile.subsector_id;
        }
        else if (layer_mode == 3)
        { // Subsector First (fallback to sector)
            ...
            for (auto &inst : map_main->sector_instances)
            {
                if (inst.id == target_inst_id)
                {
                    if (tag_id >= 0 && tag_id < (int)inst.active_tags.size())
                    {
                        ...
                        if (action == 0)
                            inst.active_tags[tag_id] = false;
                        else if (action == 1)
                            inst.active_tags[tag_id] = true;
                        else if (action == 2)
                            inst.active_tags[tag_id] = !inst.active_tags[tag_id];
        ...
        if (!undo)
        {
            if (creature->bars[(int)effect.parameter3].active)
            {
                ...
                if (distribution <= 0.0f)
                    distribution = 0.01f; // Prevent division by zero
    ...
    break;
    case 72: // REPEATED LOOT ROLL BASED ON BAR
    {
        int tableID = (int)effect.parameter1;
        int barIndex = (int)effect.parameter2;
        float scatter = effect.parameter3;

        // If a creature is involved, we use its bar to determine the count
        int rollCount = 1;
        if (creature != NULL && barIndex >= 0)
        {
            rollCount = (int)creature->bars[barIndex].value;
        }
        ...
        {
            int itemID = roll_loot(tableID);

            if (itemID != -1)
            {
                float offsetX = (float)(rand() % (int)scatter) - (scatter / 2.0f);
                float offsetY = (float)(rand() % (int)scatter) - (scatter / 2.0f);

                // FIXED: Use the 'create_item' function and the x/y arguments
                create_item(map_main, itemID, 1, x + offsetX, y + offsetY, -1);
            }
        }
        return_value = true;
    }
    break;
    case 82:
        // 82=drop BOUND plot_object parameter1, size parameter2, area parameter3, max nearby parameter4
        if (!undo)
        {
            int new_item_idx = -1;
            bool can_drop = true;

            // 1. Calculate area and density requirements
            if (effect.parameter3 > 0 && effect.parameter4 > 0)
            {
                int alku_x = (int)((x - effect.parameter3) / grid_size) - 3;
                int alku_y = (int)((y - effect.parameter3) / grid_size) - 3;
                int loppu_x = (int)((x + effect.parameter3) / grid_size) + 3;
                ...
                for (int a = alku_x; a < loppu_x; a++)
                {
                    for (int b = alku_y; b < loppu_y; b++)
                    {
                        for (int c = 0; c < map_main->at(a, b).items.size(); c++)
                        {
                            int idx = map_main->at(a, b).items[c];
                            if (map_main->items[idx].base_type != 0 || map_main->items[idx].dead)
                                continue;

                            float size = map_main->items[idx].size * general_object_size;
                            // Check distance
                            if (map_main->items[idx].x + size * 0.5f < x - effect.parameter3)
                                continue;
                            if (map_main->items[idx].x + size * 0.5f > x + effect.parameter3)
                                continue;
                            if (map_main->items[idx].y + size * 0.5f < y - effect.parameter3)
                                continue;
                            if (map_main->items[idx].y + size * 0.5f > y + effect.parameter3)
                                continue;

                            if (map_main->items[idx].item_type == (int)effect.parameter1)
                                objects_here++;
                        }
                    }
                }
            ...
            if (can_drop)
            {
                ...
                // Add random offset if area size is set
                if (effect.parameter3 > 0)
                {
                    spawn_x += randDouble(-effect.parameter3, effect.parameter3);
                    spawn_y += randDouble(-effect.parameter3, effect.parameter3);
                }

                // Create the object
                create_plot_object(map_main, (int)effect.parameter1, spawn_x, spawn_y, angle, effect.parameter2);

                // 3. Bind to parent: Retrieve the index of the newly created item
                // (Assumes create_plot_object adds to the end of map_main->items)
                new_item_idx = (int)map_main->items.size() - 1;

                if (new_item_idx >= 0 && creature_number != -1)
                {
                    // Bind the creature ID to the item
                    map_main->items[new_item_idx].parent_creature_id = creature_number;
                }
            }
...
void game_engine::draw_item_view(void)
{
    ...
    for (int a = 0; a < inventory[active_inventory].player_items.size(); a++)
    {
        ...
        if (inventory_entry.item <= -1)
            continue;
        if (inventory_entry.amount <= 0)
            continue;
        if (mod.general_items[inventory_entry.item].visible_in_inventory == 0)
            continue;
        // if(inventory_entry.wielded)continue;
        total_items++;
        if (total_items >= mahtuu)
        {
            rullaus = true;
        }
        if (rivi >= mahtuu)
        ...
        if ((mousex > item_dialog_x + 83) && (mousex < item_dialog_x + 240 + right_line - 10) && (mousey >= item_dialog_y + item_list_y + rivi * rivi_korkeus) && (mousey < item_dialog_y + item_list_y + (rivi + 1) * rivi_korkeus))
        {
        ...
        if ((mouse_on_this) && (combine_item != -1))
        {
            ...
            if (item_has_combination(inventory[active_inventory].player_items[combine_item].item, inventory_entry.item, &combination))
                combination_found = true;
            if (item_has_combination(inventory_entry.item, inventory[active_inventory].player_items[combine_item].item, &combination))
                combination_found = true;
    ...
    else if (((item_description == -1) || (item_description == -2)) && (mod.general_races[player_race].rag_doll >= 0))
    {
        ...
        for (int a = 0; a < mod.general_races[player_race].slots.size(); a++)
        {
            ...
            if (races_entry.active)
            {
                ...
                // mouse on it
                bool mouse_on = false;
                if ((mousex > item_dialog_x + rag_doll_x + races_entry.x) && (mousey > item_dialog_y + rag_doll_y + races_entry.y) && (mousex < item_dialog_x + rag_doll_x + races_entry.x + 64) && (mousey < item_dialog_y + rag_doll_y + races_entry.y + 64))
                {
                    mouse_on = true;
                    mouse_on_slot = true;
                    // place item here
        ...
        for (int a = 0; a < inventory[active_inventory].player_items.size(); a++)
        {
            ...
            for (int b = 0; b < mod.general_items[inventory_entry.item].wield_slots.size(); b++)
            {
                ...
                // mouse on it
                if ((mousex > item_dialog_x + rag_doll_x + mod.general_races[player_race].slots[mod.general_items[inventory_entry.item].wield_slots[b]].x) && (mousey > item_dialog_y + rag_doll_y + mod.general_races[player_race].slots[mod.general_items[inventory_entry.item].wield_slots[b]].y) && (mousex < item_dialog_x + rag_doll_x + mod.general_races[player_race].slots[mod.general_items[inventory_entry.item].wield_slots[b]].x + 64) && (mousey < item_dialog_y + rag_doll_y + mod.general_races[player_race].slots[mod.general_items[inventory_entry.item].wield_slots[b]].y + 64))
                {
                    if (!mouse_left && mouse_left2)
                    {
                        mouse_left = false;
    ...
    if (item_description != -1)
    {
        ...
        {
            grim->Quads_SetColor(1, 1, 1, 1);
            text_manager.write(font, mod.general_items[item_description].name, 1.1f,
                               item_dialog_x + 250 + right_line, item_dialog_y + 55 + 30,
                               item_dialog_x + 640 - 20, screen_height, 1, 1, 1, 1);
            ...
            float current_y = item_dialog_y + 220; // Starting Y below description
            float small_scale = 0.8f;              // Smaller text scale

            // --- PART A: USE KEYS ---
            grim->Quads_SetColor(0.8f, 1.0f, 0.8f, 0.8f); // Header color
            text_manager.write_line(font, item_dialog_x + 250 + right_line, current_y, "Use keys:", small_scale);
            current_y += 15;

            for (int e = 0; e < mod.general_items[item_description].effects.size(); e++)
            {
                auto &ef = mod.general_items[item_description].effects[e];
                // Ignore if key is '-' or 'none'
                if (ef.use_key != '-' && ef.use_key != 'n' && ef.use_key != -1)
                {
                    std::string line = "()";
                    line += ef.use_key;
                    line += ") - ";
                    line += ef.key_description;

                    grim->Quads_SetColor(1, 1, 1, 1);
                    // Use write_line to prevent wrapping
                    text_manager.write_line(font, item_dialog_x + 260 + right_line, current_y, line, small_scale);
                    current_y += 14;
                }
            }
            ...
            // --- PART B: QUICK KEYS ---
            grim->Quads_SetColor(0.8f, 1.0f, 0.8f, 0.8f);
            text_manager.write_line(font, item_dialog_x + 250 + right_line, current_y, "Quick keys:", small_scale);
            current_y += 15;

            for (int e = 0; e < mod.general_items[item_description].effects.size(); e++)
            {
                auto &ef = mod.general_items[item_description].effects[e];
                // Ignore if key is '-' or '0' or 'none'
                if (ef.quick_key != '-' && ef.quick_key != 'n' && ef.quick_key != 0 && ef.quick_key != -1)
                {
                    std::string line = "(";
                    line += ef.quick_key;
                    line += ") - ";
    ...
    if (dismantle_item >= 0)
    {
        ...
        {
            if (mod.general_items[a].dead)
                continue;
            for (int b = 0; b < mod.general_items[a].combinations.size(); b++)
            {
                if (mod.general_items[a].combinations[b].combine_results.size() > 1)
                    continue;
                if (mod.general_items[a].combinations[b].combine_results[0].combines_amount > 1)
                    continue; // cannot uncombine if more than one was result
                if (mod.general_items[a].combinations[b].can_be_broken_up == 0)
                    continue;
    ...
    if (drop_item >= 0)
    {
        ...
        for (int a = 0; a < mod.general_races[player_race].specialties.size(); a++)
        {
            ...
            if (!specialty.difficulty[game_difficulty_level])
                continue;
            // specialty found
            bool cannot_drop = (specialty.number == 4 && specialty.parameter0 == inventory[active_inventory].player_items[drop_item].item) || (specialty.number == 0 && specialty.parameter0 == mod.general_items[inventory[active_inventory].player_items[drop_item].item].item_class);
...
bool game_engine::record_message(int type, int parameter0)
{
    ...
    if (type == 0)
    {
        if (!show_journal(parameter0, player_race))
            return false;
    }
...
void game_engine::show_text_window(int entry_number)
{
    ...
    if (entry_number < 0)
        return;
    if (entry_number >= journal_records.size())
        return;
...
void game_engine::draw_targeting_beam(void)
{
    ...
    if (beam_type.parameter1 == 2)
    {
        ...
        {
            if (collisions[a].type == 0 && collisions[a].subtype > 0 && map_main->creature[collisions[a].subtype].side != 0 && mod.general_creatures[map_main->creature[collisions[a].subtype].type].particle_on_radar >= 0)
            {
                enemy_found = true;
            }
        }
...
int game_engine::arrange_item_list_callback(const void *c, const void *d)
{
    ...
    if (a->item >= engine->mod.general_items.size())
    {
        return -1;
    }
    if (b->item >= engine->mod.general_items.size())
    {
        return 1;
    }
    ...
    {
        if (engine->mod.general_items[a->item].identifier == engine->mod.general_items[b->item].identifier)
            return 0;
        if (engine->mod.general_items[a->item].identifier > engine->mod.general_items[b->item].identifier)
            return 1;
        if (engine->mod.general_items[a->item].identifier < engine->mod.general_items[b->item].identifier)
    ...
    if (!weapon_1 && !weapon_2)
    {
        ...
        {
            if (engine->mod.general_items[a->item].name > engine->mod.general_items[b->item].name)
                return 1;
            if (engine->mod.general_items[a->item].name < engine->mod.general_items[b->item].name)
                return -1;
            return 0;
...
void game_engine::render_credits(void)
{
    ...
    */
    ...
    const string credits("\\ Contributors: \\" CONTRIBUTORS " \\"
...
void game_engine::new_game(void)
{
    ...
    for (a = 0; a < mod.general_races[player_race].specialties.size(); a++)
    {
        ...
        if (!mod.general_races[player_race].specialties[a].difficulty[game_difficulty_level])
            continue;
...
void game_engine::render_menu(void)
{
    ...
    for (a = 0; a < menu_system[submenu].items; a++)
    {
        // mouse on this
        bool mouse_on_this = false;
        if ((mousex > menu_system[submenu].start_x / 1024.0f * screen_width) && (mousex < menu_system[submenu].end_x / 1024.0f * screen_width) && (mousey > (menu_system[submenu].start_y + position) / 768.0f * screen_height) && (mousey < (menu_system[submenu].start_y + position + menu_system[submenu].item[a].height) / 768.0f * screen_height))
        {
            mouse_on_this = true;
        }
...
void game_engine::playsound(int sample_number, float volume, float sound_x, float sound_y, float listener_x, float listener_y)
{ // plays a sound
    ...
    if (!sound_initialized)
        return;
    if (sample_number >= resources.samples_loaded)
        return;
    if (sample_number < 0)
        return;
...
bool game_engine::creature_will_collide(map *new_map, creature_base *creature)
{
    ...
    if (creature->y < 0)
        return true;
    if (creature->x > (new_map->sizex - 1) * grid_size)
        return true;
    if (creature->y > (new_map->sizey - 1) * grid_size)
        return true;
    ...
    if (alku_x < 0)
        alku_x = 0;
    if (loppu_x > new_map->sizex - 1)
        loppu_x = new_map->sizex - 1;
    if (alku_y < 0)
        alku_y = 0;
    if (loppu_y > new_map->sizey - 1)
        loppu_y = new_map->sizey - 1;
    ...
    for (a = alku_x; a < loppu_x; a++)
    {
        for (b = alku_y; b < loppu_y; b++)
        {
            ...
            if (!prop_prevented)
                ...
                {
                    if (new_map->object[new_map->at(a, b).objects[c]].dead)
                        continue;
            ...
            if (!plot_object_prevented)
                ...
                {
                    if (new_map->items[new_map->at(a, b).items[c]].dead)
                        continue;
                    if (new_map->items[new_map->at(a, b).items[c]].base_type != 0)
                        continue;
...
bool game_engine::point_will_collide(map *new_map, float x, float y, bool only_ones_that_stop_bullets)
{
    ...
    if (y < 0)
        return true;
    if (x > (new_map->sizex - 1) * grid_size)
        return true;
    if (y > (new_map->sizey - 1) * grid_size)
        return true;
        ...
        for (b = alku_y; b < loppu_y; b++)
        {
            ...
            for (c = 0; c < new_map->at(a, b).objects.size(); c++)
            {
                ...
                if (only_ones_that_stop_bullets)
                    if (!mod.general_objects[new_map->object[new_map->at(a, b).objects[c]].type].stops_bullets)
                        continue;
            ...
            for (c = 0; c < new_map->at(a, b).items.size(); c++)
            {
                ...
                if (new_map->items[new_map->at(a, b).items[c]].base_type != 0)
                    continue;
                if (only_ones_that_stop_bullets)
                    if (!mod.general_objects[new_map->items[new_map->at(a, b).items[c]].type].stops_bullets)
                        continue;
                // if(creature_collision_detection(creature,&new_map->items[new_map->at(a,b).items[c]],false))
...
void game_engine::create_maps(void)
{
    ...
    for (a = 0; a < mod.general_plot_objects.size(); a++)
    {
        if (mod.general_plot_objects[a].dead)
            continue;
        ...
        for (int b = 0; b < mod.general_plot_objects[a].amount; b++)
        {
            ...
            while (area_unfit && (tries < 1000))
            {
                ...
                if (mod.general_areas[area_number].dead)
                    continue;
    ...
    for (d = 0; d < mod.general_areas.size(); d++)
    {
        ...
        for (a = 0; a < temp_map->items.size(); a++)
        {
            ...
            if (temp_map->items[a].base_type != 0)
                continue;
            if (temp_map->items[a].event_used)
                continue;
            if (temp_map->items[a].dead)
                continue;
            ...
            if (mod.general_plot_objects[temp_map->items[a].item_type].trigger_event_by == 3)
                for (c = 0; c < mod.general_plot_objects[temp_map->items[a].item_type].effects.size(); c++)
                {
                    ...
                    if ((mod.general_plot_objects[temp_map->items[a].item_type].effects[c].vanish_after_used == 0) || (mod.general_plot_objects[temp_map->items[a].item_type].effects[c].vanish_after_used == 1))
                    { // cannot be reused
        ...
        if (map_main->rain_timer > 0)
        {
            ...
            if (!in_shade)
            {
                ...
                for (a = 0; a < mod.general_climates[map_main->climate_number].rain_effects.size(); a++)
                {
                    ...
                    if (time_from_beginning - rain_effect_timers[map_main->climate_number].subtype[a] < mod.general_climates[map_main->climate_number].rain_effects[a].interval * 0.001f)
                        continue;
    ...
    for (i = alku_x; i < loppu_x; i++)
    {
        for (j = alku_y; j < loppu_y; j++)
        {
            if (map_main->at(i, j).total_creatures > 0)
                for (k = 0; k < map_main->at(i, j).total_creatures; k++)
                {
                    if (map_main->creature[map_main->at(i, j).creatures[k]].dead)
                        continue;
...
void game_engine::spawn_creature(int side, int tactic, int tactic2, float x, float y, float angle, int type, map *map)
{
    ...
    if (tactic == -1)
        temp_creature.tactic[0] = mod.general_creatures[temp_creature.type].AI_tactics[0];
    temp_creature.tactic[1] = tactic2;
    if (tactic2 == -1)
        temp_creature.tactic[1] = mod.general_creatures[temp_creature.type].AI_tactics[1];
...
void game_engine::set_edges(void)
{
    ...
    for (a = 0; a < width * height; a++)
    {
        ...
        if (map_list[a] >= 0)
        {
            ...
            if (a > 0)
                if (map_list[a - 1] >= 0)
                    map_storage[map_list[a]]->near_areas[0] = map_list[a - 1];
            // right
            if (a < width * height - 1)
                if (map_list[a + 1] >= 0)
                    map_storage[map_list[a]]->near_areas[1] = map_list[a + 1];
            // up
            if (a >= width)
                if (map_list[a - width] >= 0)
                    map_storage[map_list[a]]->near_areas[2] = map_list[a - width];
            // down
            if (a < (height - 1) * width)
                if (map_list[a + width] >= 0)
                    map_storage[map_list[a]]->near_areas[3] = map_list[a + width];
        }
    }
...
void game_engine::draw_slider(void)
{
    ...
    if (slider_type == 2)
    {
        ...
        if (slider_point <= 0.0f)
        {
            ...
            for (int a = 0; a < inventory[active_inventory].player_items.size(); a++)
            {
                if (inventory[active_inventory].player_items[a].item <= -1)
                    continue;
                if (inventory[active_inventory].player_items[a].amount <= 0)
                    continue;
                if (mod.general_items[inventory[active_inventory].player_items[a].item].visible_in_inventory == 0)
                    continue;
    ...
    if (move_by_mouse)
    {
        if ((override_mousey > slider_y + slider_bar_y) && (override_mousey < slider_y + slider_bar_y + slider_height))
        {
            ...
            {
                slider_point = static_cast<int>(((override_mousex - slider_x - slider_bar_x) / slider_length) * (slider_maximum - slider_minimum) + 0.5f); // !
...
bool game_engine::race_specialty(int find_specialty, Mod::specialty *specialty)
{
    ...
    {
        if (!mod.general_races[player_race].specialties[i].difficulty[game_difficulty_level])
            continue;
        // specialty found
...
vector<game_engine::collision_base> game_engine::list_collisions(float x1, float y1, float x2, float y2, bool only_visible_area)
{
    ...
    {
        if (xx1 < screen_start_x - 3)
            xx1 = screen_start_x - 3;
        if (xx1 >= screen_end_x + 3)
            xx1 = screen_end_x + 3;
        if (xx2 < screen_start_x - 3)
            xx2 = screen_start_x - 3;
        if (xx2 >= screen_end_x + 3)
            xx2 = screen_end_x + 3;
    }

    if (xx1 < 0)
        xx1 = 0;
    if (xx2 > map_main->sizex - 1)
        xx2 = map_main->sizex - 1;
    ...
    if (only_visible_area)
    {
        if (yy1 < screen_start_y - 3)
            yy1 = screen_start_y - 3;
        if (yy1 >= screen_end_y + 3)
            yy1 = screen_end_y + 3;
        if (yy2 < screen_start_y - 3)
            yy2 = screen_start_y - 3;
        if (yy2 >= screen_end_y + 3)
            yy2 = screen_end_y + 3;
    }
    if (yy1 < 0)
        yy1 = 0;
    if (yy2 > map_main->sizey - 1)
        yy2 = map_main->sizey - 1;
...
bool game_engine::check_condition(const Mod::condition &condition, const creature_base *creature, int creature_number, float x, float y, bool show_message, int current_weapon = -1)
{
    ...
    case 0:
    {
        ...
        if (condition.condition_parameter1 > 0)
        {
            ...
            if (!found)
            {
                if (show_message)
                {
                    if (isvowel(mod.general_items[condition.condition_parameter0].name.at(0)))
                        tempstring = "An ";
                    else
                        tempstring = "A ";
    ...
    case 1:
    {
        ...
        if (!found)
        {
            if (show_message)
            {
                ...
                if (isvowel(mod.general_plot_objects[condition.condition_parameter0].name.at(0)))
                    tempstring += "an ";
                else
                    tempstring += "a ";
    ...
    case 2:
    {
        ...
        if (!found)
        {
            ...
            {
                tempstring = "Must be near ";
                if (isvowel(mod.plot_object_classes[condition.condition_parameter0].at(0)))
                    tempstring += "an ";
                else
                    tempstring += "a ";
    ...
    {
        // no such bar
        if (!creature->bars[(int)condition.condition_parameter0].active)
            return false;
    ...
    case 4:
    {
        ...
        for (a = 0; a < map_storage.size(); a++)
        {
            if (map_storage[a]->area_type == area)
            {
                ...
                {
                    if (map_storage[a]->creature[c].dead)
                        continue;
                    if (map_storage[a]->creature[c].type != condition.condition_parameter0)
                        continue;
                    if (map_storage[a]->creature[c].killed)
                        continue;
                    if (show_message)
                    {
    ...
    case 6:
    {
        if (player_race != condition.condition_parameter0)
            return false;
    }
    break;
    ...
    case 8:
    {
        if (randInt(0, condition.condition_parameter0) != 0)
            return false;
    }
    break;
    ...
    case 10:
    {
        if (creature->eat_counter < condition.condition_parameter0)
            return false;
    }
    break;
    ...
    case 12:
    {
        if (condition.condition_parameter0 == 0)
        {
            if (game_difficulty_level <= condition.condition_parameter1)
                return false;
        }
    ...
    case 14:
    {
        ...
        for (a = start_x; a < end_x; a++)
        {
            for (b = start_y; b < end_y; b++)
            {
                for (c = 0; c < map_main->at(a, b).total_creatures; c++)
                {
                    ...
                    if (condition.condition_parameter0 >= 0)
                    {
                        if (map_main->creature[map_main->at(a, b).creatures[c]].type != condition.condition_parameter0)
                            continue;
                    }
                    else
                    {
                        if (mod.general_creatures[map_main->creature[map_main->at(a, b).creatures[c]].type].creature_class != -condition.condition_parameter0)
                            continue;
                    }
        ...
        if (!found)
        {
            ...
            {
                tempstring = "Must be near ";
                if (isvowel(mod.general_creatures[condition.condition_parameter0].name.at(0)))
                    tempstring += "an ";
                else
                    tempstring += "a ";
    ...
    {
        // no such bar
        if (!map_main->creature[0].bars[(int)condition.condition_parameter0].active)
            return false;
    ...
    case 40:
    {
        ...
        if (!creature->bars[(int)condition.condition_parameter0].active)
            return false;
        if (!creature->bars[(int)condition.condition_parameter1].active)
            return false;
            ...
            for (auto &inst : map_main->sector_instances)
            {
                if (inst.id == target_inst_id)
                {
                    if (tag_id >= 0 && tag_id < (int)inst.active_tags.size())
                    {
                        ...
                        if (required_state == 1)
                            return is_active; // Must be On
                        if (required_state == 0)
                            return !is_active; // Must be Off
                    }
                    break;
                }
            }
    ...
    // 49=players's bar parameter0 is smaller than parameter1 percent of maximum (value of 100=100%)
    case 49:
    {
        // no such bars
        if (!map_main->creature[0].bars[(int)condition.condition_parameter0].active)
        ...
        // compare
        if ((map_main->creature[0].bars[(int)condition.condition_parameter0].value / map_main->creature[0].bars[(int)condition.condition_parameter0].maximum) * 100 >= condition.condition_parameter1)
        {
    ...
    break;
    // 50=players's bar parameter0 is more than parameter1 percent of maximum (value of 100=100%)
    case 50:
    {
        // no such bars
        if (!map_main->creature[0].bars[(int)condition.condition_parameter0].active)
...
void game_engine::draw_bars(void)
{
    ...
    for (a = 0; a < maximum_bars; a++)
    {
        if (a >= mod.general_bars.size())
            continue;
        if (mod.general_bars[a].dead)
            continue;

        if (!map_main->creature[0].bars[a].active)
            continue;
        if (!mod.general_bars[a].visible)
            continue;
        ...
        if (map_main->creature[0].bars[a].value < 0)
        {
            ...
            if (map_main->creature[0].bars[a].minimum == 0)
                left_side = map_main->creature[0].bars[a].value / map_main->creature[0].bars[a].maximum * mod.general_bars[a].length;
            right_side = 0;
        ...
        if (map_main->creature[0].bars[a].value >= 0)
        {
            ...
            if (map_main->creature[0].bars[a].maximum == 0)
                right_side = map_main->creature[0].bars[a].value / map_main->creature[0].bars[a].minimum * mod.general_bars[a].length;
        ...
        if (left_side < -1 * fabs(mod.general_bars[a].length))
            left_side = -1 * fabs(mod.general_bars[a].length);
        if (right_side > 1 * fabs(mod.general_bars[a].length))
            right_side = 1 * fabs(mod.general_bars[a].length);
        ...
        if (mod.general_bars[a].handle_moodlet && (!mod.general_bars[a].moodlet_breakpoints.size()) == 0)
        {
            ...
            if (pulse_intensity > 1.0f)
                pulse_intensity = 1.0f;
    ...
    if (show_name >= 0)
    {
        float x = mousex + 20;
        float y = mousey - 8;
        if (x + mod.general_bars[show_name].name.length() * 10 > screen_width)
            x = screen_width - mod.general_bars[show_name].name.length() * 10;
...
void game_engine::count_bars(void)
{
    ...
    for (a = 0; a < mod.general_races[player_race].specialties.size(); a++)
    {
        ...
        {
            // check condition
            if (!map_main->creature[0].bars[(int)mod.general_races[player_race].specialties[a].parameter0].active)
                continue;
...
void game_engine::calculate_scripts(void)
{
    ...
    {
        if (mod.scripts[script_number].dead)
            continue;
        if (script_info[script_number].dead)
            continue;
        if (!mod.scripts[script_number].run_without_calling)
            continue;
...
void game_engine::run_script(int script_number, bool check_conditions, bool check_time)
{
    ...
    if (check_time)
    {
        ...
        if (time_increment < mod.scripts[script_number].interval)
            return;
...
void game_engine::carry_light(map *edit_map, creature_base *creature, int light)
{
    ...
    case 0:
        ...
        {
            float rot = edit_map->lights[light].rotation;
            float forward_offset = size * 0.35f; // Pushes it 35% forward from center (near the hands/gun)

            edit_map->lights[light].x2 = creature->x + size * 0.5f + sincos.table_cos(rot - pi / 2) * forward_offset;
            edit_map->lights[light].y2 = creature->y + size * 0.5f + sincos.table_sin(rot - pi / 2) * forward_offset;
        }
        break;
...
bool game_engine::run_plot_object(int item)
{
    // for all effects
    bool must_delete = false;
    // 1. Determine who the "Active" creature is for this script
    int creator_idx = map_main->items[item].parent_creature_id;
    creature_base *trigger_creature = &map_main->creature[0];

    // 2. SAFETY CHECK: Is the bound creator still valid and alive?
    if (creator_idx >= 0 && creator_idx < (int)map_main->creature.size())
    {
        // Only use the creator if they aren't dead/deleted
        if (!map_main->creature[creator_idx].dead && !map_main->creature[creator_idx].killed)
        {
            trigger_creature = &map_main->creature[creator_idx];
        }
        else
        {
            // OPTIONAL: If the creator dies, maybe the object should stop working?
            // return false;
        }
    }

    float temp_angle = trigger_creature->rotation;
    trigger_creature->rotation = map_main->items[item].rotation;
    ...
    for (a = 0; a < mod.general_plot_objects[map_main->items[item].item_type].effects.size(); a++)
    {
        ...
        {
            if (!check_condition(mod.general_plot_objects[map_main->items[item].item_type].effects[a].conditions[b],
                                 trigger_creature, creator_idx, // Pass the correct creature and its ID
                                 map_main->items[item].x + size * 0.5f,
                                 map_main->items[item].y + size * 0.5f,
                                 mod.general_plot_objects[map_main->items[item].item_type].show_condition_help, -1))
            {
                OK = false;
                break;
            }
        ...
        // run effects
        float temp_x2 = trigger_creature->x2;
        float temp_y2 = trigger_creature->y2;
        trigger_creature->x2 = trigger_creature->x;
        trigger_creature->y2 = trigger_creature->y;
        bool effect_ran = true;
        for (b = 0; b < mod.general_plot_objects[map_main->items[item].item_type].effects[a].effects.size(); b++)
        {
            if (!run_effect(effect.effects[b], trigger_creature, creator_idx,
                            map_main->items[item].x + size * 0.5f,
                            map_main->items[item].y + size * 0.5f,
                            map_main->items[item].rotation, false))
            {
                break;
            }
        }
        trigger_creature->x2 = temp_x2;
        trigger_creature->y2 = temp_y2;
        ...
        if (effect_ran)
        {
            ...
            // delete item if needed
            if ((effect.vanish_after_used == 0) || (effect.vanish_after_used == 1))
            { // cannot be reused
    ...
    trigger_creature->rotation = temp_angle;
    return must_delete;
}
        ...
        for (b = alku_y; b < loppu_y; b++)
        {
            ...
            if (check_props)
                for (c = 0; c < map_main->at(a, b).objects.size(); c++)
                {
                    ...
                    if (only_ones_that_hinder_visibility)
                        if (!mod.general_objects[map_main->object[map_main->at(a, b).objects[c]].type].blocks_vision)
                            continue;
                    if (only_ones_that_stop_bullets)
                        if (!mod.general_objects[map_main->object[map_main->at(a, b).objects[c]].type].stops_bullets)
                            continue;
            ...
            if (check_plot_objects)
                for (c = 0; c < map_main->at(a, b).items.size(); c++)
                {
                    if (map_main->items[map_main->at(a, b).items[c]].dead)
                        continue;
                    if (map_main->items[map_main->at(a, b).items[c]].type >= mod.general_objects.size())
                        continue;
                    if (only_ones_that_hinder_visibility)
                        if (!mod.general_objects[map_main->items[map_main->at(a, b).items[c]].type].blocks_vision)
                            continue;
                    if (only_ones_that_stop_bullets)
                        if (!mod.general_objects[map_main->items[map_main->at(a, b).items[c]].type].stops_bullets)
                            continue;
                    if (map_main->items[map_main->at(a, b).items[c]].base_type != 0)
                        continue;
...
void game_engine::find_suggested_camera_position(float *suggested_camera_x, float *suggested_camera_y)
{
    ...
    { // Standard Player Follow
        ...
        if (distFromCenter > deadZone)
        {
            ...
            if (lookDist > maxLookDistance)
                lookDist = maxLookDistance;
...
void game_engine::draw_map_grid_small(map *map_to_edit, int texture, int texture2)
{ // renders map grid
...
void game_engine::draw_shadow_map_system()
{
    ...
    for (int i = 0; i < (int)map_main->lights.size(); i++)
    {
        ...
        { // 0 = Flashlight
            ...
            float forward_amount = radius * 0.5f;
    ...
    for (int i = 0; i < (int)map_main->lights.size(); i++)
    {
        ...
        if (L_template.type == 0)
        {
            ...
            float forward_amount = base_radius * 0.5f;
            if (forward_amount < 0.0f)
                forward_amount = 0.0f;
...
void game_engine::draw_shadow_geometry(float lightX, float lightY, float radius)
{
    ...
    {
        if (type < 0 || !mod.general_objects[type].blocks_vision)
            return;
        ...
        if (coll_type == 0)
        {
            ...
            if (dist_sq <= radius * radius)
                return; // Light inside the circle
            ...
            float shift = 2.0f;
            float sx1 = x1 + (vx1 / len1) * shift;
            float sy1 = y1 + (vy1 / len1) * shift;
            float sx2 = x2 + (vx2 / len2) * shift;
            float sy2 = y2 + (vy2 / len2) * shift;
        ...
        else if (coll_type == 1)
        {
            ...
            for (size_t a = 0; a < poly.points.size() - 1; a++)
            {
                ...
                if (((midX - lightX) * nx + (midY - lightY) * ny) < 0)
                    continue;

                // CONNECTIVITY CHECK: Skip edges that are flush against another wall
                if (point_will_collide(map_main, midX + nx * 5.0f, midY + ny * 5.0f, true))
                    continue;
                ...
                float shift = 2.0f;
                float sx1 = x1 + (dx1 / len1) * shift;
                float sy1 = y1 + (dy1 / len1) * shift;
                float sx2 = x2 + (dx2 / len2) * shift;
                float sy2 = y2 + (dy2 / len2) * shift;
    ...
    for (int i = sx; i <= ex; i++)
    {
        for (int j = sy; j <= ey; j++)
        {
            auto &grid_pt = map_main->at(i, j);
            for (auto &idx : grid_pt.objects)
                cast_shadow(map_main->object[idx].type, map_main->object[idx].x, map_main->object[idx].y, map_main->object[idx].size, map_main->object[idx].rotation);
            for (auto &idx : grid_pt.items)
                if (map_main->items[idx].base_type == 0)
                    cast_shadow(map_main->items[idx].type, map_main->items[idx].x, map_main->items[idx].y, map_main->items[idx].size, map_main->items[idx].rotation);
        }
    }
}
int game_engine::roll_loot(int tableId)
{
    // 1. Access loot_tables from the 'mod' member defined in main.h
    int index = -1;
    for (int i = 0; i < (int)mod.loot_tables.size(); i++)
    {
        if (mod.loot_tables[i].id == tableId)
        {
            index = i;
            break;
        }
    ...
    // Use the Mod namespace version of the struct
    Mod::LootTable &t = mod.loot_tables[index];
...
void game_engine::draw_creature_dialogs()
{
    // Same culling logic as draw_map_creatures to find visible units
    int alku_x = (int)(camera_x / grid_size) - 1;
    int alku_y = (int)(camera_y / grid_size) - 1;
    int loppu_x = alku_x + (int)(screen_width / grid_size) + 4;
    int loppu_y = alku_y + (int)(screen_height / grid_size) + 4;
    ...
    for (int i = alku_x; i <= loppu_x; i++)
    {
        for (int j = alku_y; j <= loppu_y; j++)
        {
            for (int k = 0; k < map_main->at(i, j).total_creatures; k++)
            {
                int creature_idx = map_main->at(i, j).creatures[k];
                creature_base &c = map_main->creature[creature_idx];

                // Only draw dialog if creature is alive and has a message
                if (!c.killed && c.dialog >= 0)
                {
                    float size = mod.general_creatures[c.type].size * c.size * general_creature_size;
                    grim->Quads_SetRotation(0);

                    // Position the bubble relative to the creature
                    float text_x = -camera_x + c.x + size * 0.8f + 5;
                    float text_y = -camera_y + c.y + size * 0.5f - 6;

                    // Force alpha to 1.0 here so text is always readable
                    text_manager.write(-1, mod.dialogs[c.dialog].text, 1,
                                       text_x, text_y, text_x + 300, screen_height - 10,
                                       mod.dialogs[c.dialog].r,
                                       mod.dialogs[c.dialog].g,
                                       mod.dialogs[c.dialog].b, 1.0f);
                }
            }
        }
    }
